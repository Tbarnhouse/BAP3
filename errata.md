...
| 32 | There is a typo in the numerator of the normalizing constant | it should be $p(\theta) = \underbrace{\frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) \Gamma(\beta)}}_{\text{normalizing constant}} \theta^{\alpha-1} (1-\theta)^{\beta-1}$ | XIN Hongwei |
